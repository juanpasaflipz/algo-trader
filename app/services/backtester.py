...
from app.services.strategies.ema_crossover import EMACrossoverStrategy, EMACrossoverParameters
...
class Backtester(LoggerMixin):
    ...
    async def run_backtest(self, request: BacktestRequest) -> BacktestResult:
        ...
        try:
            ...
            result.trades = [self._convert_trade_to_record(t) for t in backtest_results['trades']]
    ...
    def _create_strategy(self, request: BacktestRequest) -> BaseStrategy:
        ...
        if request.strategy.upper() == "EMA_CROSSOVER":
            ...
            from app.services.strategies.ai_enhanced_strategy import AIEnhancedStrategy, AIEnhancedStrategyParameters
            params = AIEnhancedStrategyParameters(
                symbol=request.symbol,
                timeframe="1h",
                lookback_period=100,
                base_strategy="EMA_CROSSOVER",
                base_strategy_params=request.strategy_params.get("base_strategy_params", {}),
                ai_confidence_threshold=request.strategy_params.get("ai_confidence_threshold", 70.0),
                use_ai_risk_assessment=request.strategy_params.get("use_ai_risk_assessment", True),
    ...
    def _execute_backtest(
        ...
        for trade in trades:
            if trade['direction'] == 'long':
                trade['pnl'] = trade['quantity'] * (trade['exit_price'] - trade['entry_price']) - trade['commission']
            else:
                trade['pnl'] = trade['quantity'] * (trade['entry_price'] - trade['exit_price']) - trade['commission']
            
            trade['return_pct'] = trade['pnl'] / (trade['quantity'] * trade['entry_price']) * 100
    ...
    def _calculate_metrics(
        ...
        if len(negative_returns) > 0 and negative_returns.std() > 0:
            sortino_ratio = np.sqrt(252) * equity_df['returns'].mean() / negative_returns.std()
        else:
            sortino_ratio = 0
        ...
        average_trade_duration = trades_df['duration'].mean().total_seconds() / 3600  # in hours
        
        # Market exposure
        total_time = (equity_df['timestamp'].iloc[-1] - equity_df['timestamp'].iloc[0]).total_seconds()
