...
class AIWebhookProcessor(LoggerMixin):
    ...
    async def process_webhook_with_ai(
        ...
        try:
            ...
            ai_analysis = await self.ai_service.analyze_trade_signal(alert, market_context)
            ...
            if alert.quantity and alert.stop_loss:
                risk_assessment = await self._assess_trade_risk(
                    alert,
                    account_balance
                )
    ...
    async def _assess_trade_risk(
        ...
        assessment = await self.ai_service.assess_risk_parameters(
            ...
            take_profit=abs(alert.take_profit - alert.price) if alert.take_profit else stop_loss_distance * 2,
