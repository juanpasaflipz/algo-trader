...
async def get_market_commentary(
    request: MarketCommentaryRequest,
) -> MarketCommentaryResponse:
    """
    ...
    try:
        ...
        commentary = await ai_service.generate_market_commentary(
            request.symbol, request.timeframe, price_data, indicators
        )

        # Determine sentiment based on recent price action
        price_change = (
            price_data["close"].iloc[-1] / price_data["close"].iloc[-24] - 1
        ) * 100
        sentiment = (
            "bullish"
            if price_change > 1
...
async def assess_trade_risk(request: RiskAssessmentRequest) -> RiskAssessmentResponse:
    ...
    try:
        ...
        reward_amount = request.position_size * (
            request.take_profit - request.entry_price
        )
...
async def optimize_strategy_parameters(
    ...
    try:
        ...
        optimization = await ai_service.optimize_strategy_parameters(
            request.strategy_name, request.current_parameters, recent_performance
        )
        ...
        return StrategyOptimizationResponse(
            ...
            expected_improvement_percent=optimization.get(
